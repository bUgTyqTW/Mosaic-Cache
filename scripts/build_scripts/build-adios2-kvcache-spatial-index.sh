...
build_cache() {
    ...
    SPATIALINDEX_DIR="${SW_DIR}"/libspatialindex-2.1.0/
    if [ ! -d "${SPATIALINDEX_DIR}" ]
    then
        cd "${SW_DIR}" || exit
        curl -L -o libspatialindex-2.1.0.tar.gz https://github.com/libspatialindex/libspatialindex/archive/refs/tags/2.1.0.tar.gz
        tar -xzf libspatialindex-2.1.0.tar.gz
        rm libspatialindex-2.1.0.tar.gz
        cd "${SPATIALINDEX_DIR}" || exit
        mkdir build && cd build || exit
        cmake .. -DCMAKE_INSTALL_PREFIX="${SW_DIR}"/libspatialindex
        make -j32
        make install
    fi

    SPATIALINDEX_CMAKE_DIR="${SW_DIR}/libspatialindex/lib/cmake/libspatialindex"
    if [ ! -f "${SPATIALINDEX_CMAKE_DIR}/SpatialIndexConfig.cmake" ]; then
        ln -s "${SPATIALINDEX_CMAKE_DIR}/libspatialindexConfig.cmake" \
              "${SPATIALINDEX_CMAKE_DIR}/SpatialIndexConfig.cmake"
    fi
    ...
    cmake .. -DADIOS2_USE_KVCACHE=ON \
            -DADIOS2_USE_SpatialIndex=ON \
            -DADIOS2_USE_Python=ON \
            -DBUILD_TESTING=ON \
            -DCMAKE_PREFIX_PATH="${SW_DIR}/hiredis;${SPATIALINDEX_CMAKE_DIR}" \
